...
func (sw *schedWorker) startProcessingTask(taskDone chan struct{}, req *workerRequest) error {
	...
	go func() {
		...
		err = w.active.withResources(sw.wid, w.info.Resources, needRes, &sh.workersLk, func() error {
			...
			// 记录主机处理的扇区信息
			_, ok := sh.handledSector[sw.worker.info.Hostname]
			if !ok {
				sh.handledSector[sw.worker.info.Hostname] = make(map[string]storage.SectorRef)
			}

			if sealtasks.TTAddPiece == req.taskType || sealtasks.TTCommit1 == req.taskType || sealtasks.TTPreCommit1 == req.taskType || sealtasks.TTPreCommit2 == req.taskType { // 如果是C2就不用再记录
				hasSaved := false
